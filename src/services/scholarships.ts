...
function rowToScholarship(row: any, checklist: ChecklistItem[] = [], eligibleUniversities: string[] = []): Scholarship {
  return {
    id: row.id,
    name: row.name,
...
export async function getScholarships(): Promise<Scholarship[]> {
  ...
  (checklistRows ?? []).forEach((row) => {
    if (!checklistMap[row.scholarship_id]) checklistMap[row.scholarship_id] = [];
  ...
  return schols.map((s) => rowToScholarship(s, checklistMap[s.id] ?? [], uniMap[s.id] ?? []));
}
...
export async function updateScholarship(
  ...
  if (Object.keys(updateData).length > 0) {
    const { error } = await supabase.from('scholarships').update(updateData).eq('id', id);
    if (error) throw error;
  }
...
export async function deleteScholarshipChecklistItem(id: string): Promise<void> {
  const { error } = await supabase.from('scholarship_checklist').delete().eq('id', id);
  if (error) throw error;
}
